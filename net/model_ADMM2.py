...
class ADMM(torch.nn.Module):
    def __init__(self, mode = "only_tv", iter = 200, sp_file = None):
        ...
        self.inv_item = torch.linalg.inv(self.q * self.ATA + self.gamma_tv * self.DeltaTDelta + (self.alpha + self.beta + self.gamma_l1) * I)
    ...
    def update_x(self, b, u_l1, eta_l1, u_tv, eta_tv, u_dwt, eta_dwt, z, rho, w, tau, v, p):
        ...
        # resiual += z_tv
        resiual += torch.matmul(self.q * v - p, self.A)
    ...
    def update_v(self, x, b, p):
        return (self.q * torch.matmul(self.A, x) + p + b) /(1 + self.q)

    def update_p(self,x,v,p):
        p += torch.mul(self.q, (torch.matmul(self.A, x) - v))
        return p
    ...
    def forward(self, target):
        ...
        for i in range(self.iter):
            ...
            v = self.update_v(x, b, p)
            x = self.update_x(b, u_l1, eta_l1, u_tv, eta_tv, u_dwt, eta_dwt, z, rho, w, tau, v, p)
